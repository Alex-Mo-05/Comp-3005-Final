...
# show a list of all rooms and their current bookings, join tables room and pt sessions, room and group classes, show room id, type of room, type of activity, capacity of room, capacity of class, date, time, option to rebook
    while True:
        ...
        if choice == "1":
            ...
            try:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    cur.execute("SELECT repair_id, machine_id, assigned_to, reported_by, issue_description, status, date_reported FROM repairs WHERE status = 'maintenance';")
                    rows = cur.fetchall()
            ...
            cur.execute("SELECT machine_id FROM repairs WHERE repair_id = %s;", (fid))
            mid = cur.fetchone()
            if mid is not None:
                try: 
...
# mark reapirs as compleye - this should change machines back to operational
    while True:
        try:
            with conn.cursor(cursor_factory=RealDictCursor) as cur:
                cur.execute("SELECT machine_id, room_id, name, status, last_maintenance_date FROM equipment;")
                rows = cur.fetchall()
                print("\nPrinting all Machines\n")
                for r in rows:
                    print(f"  - Machine ID {r['machine_id']}: Room ID: {r['room_id']} | Name: {r['name']} | Operational Status: {r['status']} | Last Maintenance Date: {r['last_maintenance_date']}\n")
        except Exception as e:
        ...
        print("2. Assign Repairs")
        print("3. Mark Repair as Complete")
        print("4. Exit")
        choice = input("Select option: ").strip()
        if choice == "1":
                ...
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if cur.fetchone() is not None:
                        ...
                        try:
                            with conn.cursor() as cur:
                                cur.execute("""
                                    INSERT INTO repairs (machine_id, reported_by, issue_description, status, date_reported)
                                    VALUES ( %s, %s, %s, %s, %s)
                        ...
                        try:
                            with conn.cursor() as cur:
                                ...
                                conn.commit()
                                print("[Success] Machine set to down.")
            ...
            try:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if cur.fetchone() is not None:
                        ...
                        if cur.fetchone() is not None:
                            try: 
                                cur.execute("""UPDATE repairs SET assigned_to = %s WHERE repair_id = %s """, (tid, rid))
                                conn.commit()
            ...
            if mid is not None:
                try: 
                    with conn.cursor(cursor_factory=RealDictCursor) as cur:
                        cur.execute("""UPDATE repairs SET status = 'completed' WHERE repair_id = %s """, (fid))
                        cur.execute("""UPDATE equipment SET status = 'operational', last_maintenance_date = %s WHERE machine_id = %s """, (datetime.now(), mid))
                        conn.commit()
                        print("[Success] Repair updated.")
