...
def view_schedule(conn, trainer_id):
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            ...
            FROM pt_sessions
            WHERE trainer_id = %s AND status = 'open'
            ORDER BY date ASC, start_time ASC;
            """
            params = (trainer_id,)        
            cur.execute(sql, params)
            ...
            print("\nCurrently Non-Booked Physical Training Sessions:")
            if not rows:
                print("(no non-booked sessions found)\n")
                return

            for r in rows:
                print(f"  - Room ID: {r['room_id']} {r['date']} | {r['start_time']} | {r['end_time']}\n")
    except Exception as e:
        print(f"[ERROR] Failed to get booked pt sessions: {repr(e)}")
        conn.rollback()
    ...
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cur:
            cur.execute("SELECT room_id, date, start_time, end_time, capacity FROM group_classes WHERE trainer_id = %s AND status = 'scheduled' ORDER BY date ASC;", (trainer_id))
            rows = cur.fetchall()
